...
pygame.display.set_caption("Geometry Dash with Level Editor")
...
pygame.draw.polygon(spike_img, WHITE, [(0, 40), (20, 0), (40, 40)])
...
def draw_level():
    for obj in level:
        if obj['type'] == 'block':
            screen.blit(block_img, (obj['x'] - editor_scroll_x, obj['y']))
        elif obj['type'] == 'spike':
            screen.blit(spike_img, (obj['x'] - editor_scroll_x, obj['y']))
...
def display_mode():
    mode_text = font.render(f"{'CUBE' if game_mode == CUBE else 'SHIP'} MODE", True, WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    if game_mode == CUBE and player_y == HEIGHT - player_size - ground_height:
                        player_y_velocity = jump_height
                    elif game_mode == SHIP:
                        player_y_velocity = jump_height / 2
                elif event.key == pygame.K_m:
                    game_mode = SHIP if game_mode == CUBE else CUBE
